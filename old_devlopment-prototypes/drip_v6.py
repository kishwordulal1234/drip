...
#  Flag emojis use Unicode regional indicator pairs which need a
#  colour-emoji font (Noto Color Emoji etc.) — most hacker terminals
#  don't have it so they render as broken boxes.
#  We use Rich-styled bold text badges instead: works everywhere.
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# Region colour for the badge text
_REGION_COLOR = {
    # Asia-Pacific — cyan
...
def _flag(cc):
    """Return a terminal-safe Rich markup country badge e.g. [bold #00BFFF]SG[/]."""
    cc = (cc or "??").upper()
    if cc == "??":
        return "[dim]??"
...
def get_exit_ip(pc_bin, pc_conf, is_v4, timeout=8):
    ...
    for url in endpoints:
        try:
            if is_v4:
                # no -q: on some proxychains4 builds -q swallows stdout too
...
def _build_ip_flow(real_ip, ri, entry_px, pi, proxies, exit_ip, ei, chain_len=None):
    lines = []
    r_flag = ri.get("flag", "[dim]??"); r_cn = ri.get("country", "?"); r_city = ri.get("city", "")
    ...
    if entry_px:
        p_flag = pi.get("flag", "[dim]??"); p_cn = pi.get("country", "?"); p_city = pi.get("city", "")
    ...
    if exit_ip:
        e_flag = ei.get("flag", "[dim]??"); e_cn = ei.get("country", "?"); e_city = ei.get("city", "")
...
def preflight(proxies, cfg, lats, countries, pc_bin, pc_conf, tor_mode, is_v4=True, chain_len=None):
    ...
    if tor_mode:
        ...
        parts  = []
        for p in sample:
            ci = countries.get(p["host"], {}); fl = ci.get("flag", "[dim]??"); cn = ci.get("country", "?")
        ...
        for i, px in enumerate(proxies[:20], 1):
            ci  = countries.get(px["host"], {}); fl = ci.get("flag", "[dim]??")
    ...
    exit_thread.start()
    exit_thread.join(timeout=30)   # must be > timeout+10 inside get_exit_ip
    ...
    # Look up real IP geo DIRECTLY (no proxy) — routing it via entry_px
    # causes ip-api.com to see the proxy's IP and return wrong/empty data.
    # Look up exit IP geo via entry_px so the request doesn't expose us.
    real_geo = lookup_countries([real_ip], via_proxy=None) if real_ip else {}
    exit_geo = lookup_countries([exit_ip], via_proxy=entry_px) if exit_ip else {}
...
def stream_with_live_log(proc, proxies, countries, cfg, ok_count, fail_count):
    ...
    def _fmt_ip(ip):
        ci = ip_info.get(ip, {})
        fl = ci.get("flag", "[dim]??"); cc = ci.get("code", "??")
