...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def write_pc_conf(proxies, cfg, tor_mode=False, chain_len=None, quick_mode=False):
    """
    quick_mode=True: used for ICMP-substituted tools (ping→nc alive checks).
    Forces strict_chain + very short timeouts so nc returns in seconds not minutes.
    Without this, dynamic_chain with 20 proxies × 8s timeout = 160s worst case.
    """
    lines = []
    clen = chain_len or cfg["chain_len"]   # e.g. 3

    if tor_mode:
        lines.append("dynamic_chain")
    elif quick_mode:
        # For alive checks: strict so we pick exactly clen proxies,
        # no retrying dead ones, and we exit fast.
    ...
    # proxy_dns in quick_mode: KEEP it — proxychains needs it to route
    # the hostname to the SOCKS5 proxy. Without it, the tool would try
    # to resolve DNS locally (which leaks DNS and may fail behind proxy).
    # The -n flag on nmap prevents reverse-lookup of the 224.x fake IP.
    if cfg["proxy_dns"]: lines.append("proxy_dns")
    # quick_mode: tighter timeouts — fail fast per proxy
    tcp_conn = 3000 if quick_mode else cfg["tcp_conn"]
    tcp_read = 6000 if quick_mode else cfg["tcp_read"]
    lines.append(f"tcp_read_time_out {tcp_read}")
    lines.append(f"tcp_connect_time_out {tcp_conn}")
    lines.append("")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  TOOL SUBSTITUTION TABLE (ping → nmap TCP equivalent)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  TOOL COMPATIBILITY TABLES
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# Tools that use ICMP/UDP/raw sockets — proxychains cannot proxy these
_TCP_ONLY_WARN = {
    "ping":        "ping uses ICMP — proxychains only intercepts TCP.",
    "ping6":       "ping6 uses ICMPv6 — not proxychainable.",
    "traceroute":  "traceroute uses UDP/ICMP — not proxychainable.",
    "tracepath":   "tracepath uses UDP — not proxychainable.",
    "arping":      "arping uses ARP frames — not proxychainable.",
    "netdiscover": "netdiscover uses ARP — not proxychainable.",
    "arp-scan":    "arp-scan uses ARP — not proxychainable.",
    "hping3":      "hping3 raw mode uses raw sockets — use: hping3 --tcp",
    "nping":       "nping defaults to ICMP — use: nping --tcp -p PORT target",
}

# Auto-substitution: swap ICMP tool → fastest TCP equivalent
#
# SPEED HIERARCHY for alive-check:
#   nc -zv -w3 target 80        ← single TCP SYN, returns in <1s  ✅ FASTEST
#   nmap -sT -Pn -p 80 -T4      ← one port, fast timeout           ✅ FAST
#   nmap -sT -Pn -p 80,443,8080 ← multiple ports                   ❌ SLOW (was used before)
#
# nc is now the default for ping — one connection, one port, instant result.
# nmap is used for traceroute (needs its own engine) and subnet scans.
_ICMP_ALTERNATIVES = {
    "ping": {
        # nmap -sT -Pn -n:
        # -sT  = TCP connect (proxychainable)
        # -Pn  = skip host discovery (don't waste time on ICMP ping)
        # -n   = no reverse DNS on 224.x fake IP (saves time)
        # -p80 = single port (fast)
        # proxychains gives nmap a fake 224.x IP via proxy_dns,
        # then intercepts the TCP connect and routes it to the real host.
        "cmd":    ["nmap", "-sT", "-Pn", "-n", "-p", "80", "-T4"],
        "reason": "nmap single-port TCP connect — -n skips reverse DNS on proxy_dns fake IP",
    },
    "ping6": {
        "cmd":    ["nmap", "-sT", "-Pn", "-n", "-6", "-p", "80", "-T4"],
        "reason": "nmap IPv6 TCP connect",
    },
    "traceroute": {
        "cmd":    ["nmap", "--traceroute", "-sT", "-Pn", "-p", "80", "--host-timeout", "10s", "-T4"],
        "reason": "TCP traceroute through proxychains",
    },
    "tracepath": {
        "cmd":    ["nmap", "--traceroute", "-sT", "-Pn", "-p", "80", "--host-timeout", "10s", "-T4"],
        "reason": "TCP traceroute through proxychains",
    ...
    "arping": {
        "cmd":    ["nmap", "-sT", "-Pn", "--disable-arp-ping", "-p", "80", "--host-timeout", "5s", "-T4"],
        "reason": "TCP scan with ARP disabled — proxychainable",
    },
    "netdiscover": {
        "cmd":    ["nmap", "-sT", "-Pn", "-p", "80,443,22", "--host-timeout", "5s", "-T4"],
        "reason": "TCP host discovery — append /24 to target for subnet scan",
    },
    "arp-scan": {
        "cmd":    ["nmap", "-sT", "-Pn", "-p", "80,443,22", "--host-timeout", "5s", "-T4"],
        "reason": "TCP subnet scan replacing ARP scan",
    },
}

def _resolve_icmp_cmd(tool_name, user_args):
    """
    Build the final command for an ICMP-substituted tool.
    All tools use nmap — the only tool that handles proxychains proxy_dns correctly.
    Returns (final_args, reason) or None.
    """
    alt = _ICMP_ALTERNATIVES.get(tool_name)
    if not alt:
        return None
    return alt["cmd"] + list(user_args), alt["reason"]

# nmap flags that require raw sockets → TCP-safe replacements
# value = (reason, replacement_flag_or_None)
_NMAP_RAW_FLAGS = {
    "-sS": ("SYN scan requires raw sockets",             "-sT"),
    "-sU": ("UDP scan cannot be proxied at all",          None),
    "-O":  ("OS detection requires raw sockets",          "-sV"),
    "-sP": ("Ping scan uses ICMP",                        "-Pn"),
    "-sn": ("Host discovery uses ICMP/ARP",               "-Pn"),
    "-PE": ("ICMP echo probe bypasses proxychains",        "-Pn"),
    "-PP": ("ICMP timestamp probe bypasses proxychains",   "-Pn"),
    "-PM": ("ICMP netmask probe bypasses proxychains",     "-Pn"),
    "-PU": ("UDP ping bypasses proxychains",               "-Pn"),
    "-PY": ("SCTP probe bypasses proxychains",             "-Pn"),
}
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  NMAP FLAG CHECKER
#  Detects raw-socket flags and auto-replaces with TCP-safe versions.
#  Shows a clear warning panel, waits 5s, then continues.
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def check_nmap_flags(tool_args):
    """
    Scan nmap arguments for raw-socket flags.
    Auto-replaces where a TCP equivalent exists, removes where it doesn't.
    Returns (fixed_args, had_warnings).

    Examples:
      nmap -sS target   → nmap -sT target   (SYN → TCP connect)
      nmap -O  target   → nmap -sV target   (OS detect → version detect)
      nmap -sU target   → nmap target       (-sU removed, no equivalent)
      nmap -sS -O -sU   → nmap -sT -sV      (all three handled)
    """
    if not tool_args or tool_args[0].lower() not in ("nmap",):
    ...
    for i, arg in enumerate(args):
        if arg not in _NMAP_RAW_FLAGS:
            continue
        reason, replacement = _NMAP_RAW_FLAGS[arg]
        if replacement:
            warnings.append(
                f"  [bold #FF0000]{arg}[/bold #FF0000]"
                f" → [bold white]{replacement}[/bold white]"
                f"  [dim]({reason})[/dim]"
            )
            args[i] = replacement
        else:
            warnings.append(
                f"  [bold #FF0000]{arg}[/bold #FF0000]"
                f" → [bold #FF0000]REMOVED[/bold #FF0000]"
                f"  [dim]({reason} — no TCP equivalent exists)[/dim]"
            )
            args[i] = None   # will be filtered out below

    # Remove None entries (flags with no replacement)
    args = [a for a in args if a is not None]

    if warnings:
        original_cmd = " ".join(tool_args)
        fixed_cmd    = " ".join(args)
        console.print(Panel(
            "[bold #FF0000]⚠️  NMAP RAW SOCKET FLAGS DETECTED[/bold #FF0000]\n\n"
            "  These flags require raw sockets and bypass proxychains entirely.\n"
            "  Your [bold #FF0000]real IP would be visible[/bold #FF0000] to the target.\n\n"
            "  [bold white]Auto-replacing with TCP-safe equivalents:[/bold white]\n\n"
            + "\n".join(warnings) + "\n\n"
            f"  [dim]original : {original_cmd}[/dim]\n"
            f"  [bold white]fixed    : {fixed_cmd}[/bold white]\n\n"
            "  [dim]Results will be slightly less detailed but will route\n"
            "  through your proxies with your real IP hidden.\n\n"
            "  For full raw-socket nmap you need a VPN (not a proxy).[/dim]\n\n"
            "  [bold white]Continuing in 5s... Ctrl+C to cancel[/bold white]",
            title="[bold #FF0000]⚠️  NMAP FLAG AUTO-FIX[/bold #FF0000]",
        ...
        try:
            ...
            console.print("[#FF0000]cancelled.[/]")
            sys.exit(0)

    return args, bool(warnings)
...
def main():
    ...
    # Initialized here — before substitution block which may set it True
    _icmp_was_substituted = False

    # ── Warn about browsers without --browser flag ────────────────
    if not _BROWSER_MODE and tool_name in _BROWSER_EXES:
        console.print(Panel(
            f"[bold #FF0000]⚠️  ANONYMITY WARNING[/bold #FF0000]\n\n"
            f"  You're running [bold white]{tool_args[0]}[/bold white] through proxychains directly.\n\n"
            f"  [bold #FF0000]This LEAKS your DNS and real IP![/bold #FF0000]\n"
            f"  [dim]proxychains cannot patch Firefox's internal DNS resolver.\n"
            f"  WebRTC, DNS-over-HTTPS, and prefetch will bypass the proxy.[/dim]\n\n"
            f"  [bold white]For true anonymity use:[/bold white]\n"
    ...
    # ── ICMP/raw-socket tool handling ────────────────────────────
    if tool_name in _TCP_ONLY_WARN:
        if tool_name in _ICMP_ALTERNATIVES:
            # Use _resolve_icmp_cmd — handles nc vs nmap, port appending,
            # and fallback if primary binary is not installed.
            resolved = _resolve_icmp_cmd(tool_name, tool_args[1:])
            if resolved is None:
                console.print(f"[#FF0000]❌ no TCP alternative found for {tool_name}[/]")
                sys.exit(1)
            cmd_new, reason = resolved
            original = " ".join(tool_args)
            replaced = " ".join(cmd_new)
            console.print(Panel(
                f"[bold #FF0000]⚠️  {tool_name.upper()} USES RAW SOCKETS — cannot be proxied[/bold #FF0000]\n\n"
                f"  [dim]Reason  : {_TCP_ONLY_WARN[tool_name]}[/dim]\n\n"
                f"  [bold white]Auto-substituting fastest TCP equivalent:[/bold white]\n"
                f"  [dim]original : {original}[/dim]\n"
                f"  [bold white]replaced : {replaced}[/bold white]\n\n"
                f"  [dim]{reason}[/dim]\n\n"
                f"  [dim]Your real IP will be hidden through the proxy chain.[/dim]\n\n"
                f"  [bold white]Continuing in 5s... Ctrl+C to cancel[/bold white]",
                title="[bold #FF0000]⚠️  ICMP → TCP AUTO-FIX[/bold #FF0000]",
                border_style="#FFAA00", padding=(0, 2)
            ))
            console.print()
            try:
            ...
            tool_args = cmd_new
            tool_name = tool_args[0].lower().split("/")[-1]
            _icmp_was_substituted = True
        else:
            # No TCP equivalent — warn clearly, real IP will leak
            console.print(Panel(
                f"[bold #FF0000]⚠️  {tool_args[0].upper()} USES RAW SOCKETS — proxychains CANNOT proxy it[/bold #FF0000]\n\n"
                f"  [dim]{_TCP_ONLY_WARN[tool_name]}[/dim]\n\n"
                f"  [bold #FF0000]YOUR REAL IP WILL BE VISIBLE TO THE TARGET.[/bold #FF0000]\n"
                f"  [dim]ICMP and UDP bypass proxychains entirely — this is a\n"
                f"  kernel limitation no wrapper can fix.\n"
                f"  For full anonymity with raw sockets you need a VPN.[/dim]\n\n"
                f"  [bold white]Continuing anyway in 5s... Ctrl+C to cancel[/bold white]",
                title="[bold #FF0000]⚠️  RAW SOCKET — NOT PROXIED[/bold #FF0000]",
                border_style="#FF0000", padding=(0, 2)
            ))
            console.print()
            try:
    ...
    # ── nmap raw-socket flag auto-fix ─────────────────────────────
    # Runs AFTER ICMP substitution so if ping→nmap happened,
    # any leftover raw flags in the nmap command are also caught.
    tool_args, _ = check_nmap_flags(tool_args)
    if tool_args:
        tool_name = tool_args[0].lower().split("/")[-1]
    ...
    if tor_mode:
        display_proxies = proxies   # empty list in tor mode
        _quick_mode = False
    elif _icmp_was_substituted:
        # Quick alive check: use only clen best proxies, strict, fast timeout
        ...
        # Dynamic: pool of extras so dead proxies can be skipped
        display_proxies = proxies[:max(clen * 6, 20)]
        _quick_mode = False

    pc_conf = write_pc_conf(display_proxies, cfg, tor_mode, quick_mode=_quick_mode)
